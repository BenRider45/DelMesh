...
Circle Triangle::computeCircumcircle() {
  ...
  // incompatible with our current method of calculating the center of our
  // circle This loop ensures we will never choose a side with a slope of 0.
  ...
  double X_Num = (B_perpM * B.midPt.V(0)) - (A_perpM * A.midPt.V(0)) +
                 (A.midPt.V(1) - B.midPt.V(1));
  double Y_Num = (pow(B_perpM, 2) * B.midPt.V(0)) -
                 (A_perpM * B_perpM * A.midPt.V(0)) +
                 B_perpM * (A.midPt.V(1) - B.midPt.V(1));
  ...
  double CircleRadius = sqrt(pow((CircCenter_X - A.a.V(0)), 2) +
                             pow((CircCenter_Y - A.a.V(1)), 2));
...
bool Triangle::checkIncircle(Point2D d) {
  ...
  Eigen::Matrix4f mat;
  mat << a.V(0), a.V(1), a.mag, 1, b.V(0), b.V(1), b.mag, 1, c.V(0), c.V(1),
      c.mag, 1, d.V(0), d.V(1), d.mag, 1;
...
int Triangle::checkOrient() {
  ...
  double det = b.V(0) * (c.V(1) - a.V(1)) + a.V(0) * (b.V(1) - c.V(1)) +
               c.V(0) * (a.V(1) - b.V(1));
