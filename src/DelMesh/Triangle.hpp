...
public:
  ...
  double computeArea() {
    return .5 * ((a.V(0) * b.V(1) + b.V(0) * c.V(1) + c.V(0) * a.V(1)) -
                 (a.V(1) * b.V(0) + b.V(1) * c.V(0) + c.V(1) * a.V(0)));
  }

  int checkOrient(); // returns 1 if oriented correctly, returns -1 if not
                     // oriented correctly, and 0 if points are colinear
  int correctOrient(); // returns 1 if points did not need correction, -1 if
